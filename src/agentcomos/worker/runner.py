...
from agentcomos.worker.tmux_pool import kill_session, start_fake_worker_session
...
def _validate_fake_invocation(invocation: dict[str, Any]) -> None:
    if invocation.get("runtime") != "hermes_tmux":
        raise ValueError("Worker Invocation runtime must be hermes_tmux for G4")
    if invocation.get("executed_by") != "controller":
        raise ValueError("Worker Invocation must be executed_by: controller")
    forbidden = set(invocation.get("forbidden") or [])
    if {"call_gm", "call_user", "deploy", "merge_git"} - forbidden:
        raise ValueError("Worker Invocation missing required forbidden actions")
...
def start_worker(invocation_path: Path, fake: bool) -> str:
    ...
    _validate_fake_invocation(invocation)
    ...
    result = start_fake_worker_session(
    ...
    job["tmux_used"] = True
    job["started_at"] = _now()
    job["tmux_command"] = result.command
    write_worker_job(run_id, job_id, job)
